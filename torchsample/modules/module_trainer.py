...
# local imports
from ._utils import (_validate_loss_input, _validate_metric_input, _validate_optimizer_input,
                     _validate_initializer_input, _standardize_user_data, _parse_num_inputs_and_targets,
                     _is_tuple_or_list, _parse_num_inputs_and_targets_from_loader, _add_regularizer_to_loss_fn)
...
class ModuleTrainer(object):
    ...
    def fit(self,
        ...
        with TQDM() as pbar:
            ...
            for epoch_idx in range(initial_epoch, num_epoch):
                ...
                if has_val_data:
                    val_epoch_logs = self.evaluate(
                        val_inputs, val_targets, batch_size=batch_size, cuda_device=cuda_device, verbose=verbose)
    ...
    def fit_loader(self, loader, val_loader=None, initial_epoch=0, num_epoch=100, cuda_device=-1, verbose=1):
        ...
        with TQDM() as pbar:
            ...
            for epoch_idx in range(initial_epoch, num_epoch):
                ...
                if has_val_data:
                    val_epoch_logs = self.evaluate_loader(val_loader, cuda_device=cuda_device, verbose=verbose)
    ...
    def predict(self, inputs, batch_size=32, cuda_device=-1, verbose=1):
        self.model.train(mode=False)
        # --------------------------------------------------------
    ...
    def predict_loader(self, loader, cuda_device=-1, verbose=1):
        self.model.train(mode=False)
        # --------------------------------------------------------
    ...
    def evaluate(self, inputs, targets=None, batch_size=32, cuda_device=-1, verbose=1):
        self.model.train(mode=False)
        num_inputs, num_targets = _parse_num_inputs_and_targets(inputs, targets)
        len_inputs = len(inputs) if not _is_tuple_or_list(inputs) else len(inputs[0])
        num_batches = int(math.ceil(len_inputs / batch_size))
        ...
        samples_seen = 0
        for batch_idx in range(num_batches):
            input_batch, target_batch = evaluate_helper.grab_batch(
                batch_idx, batch_size, inputs, targets, volatile=True)
            if cuda_device >= 0:
    ...
    def evaluate_loader(self, loader, cuda_device=-1, verbose=1):
        self.model.train(mode=False)
        num_inputs, num_targets = _parse_num_inputs_and_targets_from_loader(loader)
        batch_size = loader.batch_size
        len_inputs = len(loader.sampler) if loader.sampler else len(loader.dataset)
...
class SingleInput_SingleTarget_Helper(object):
    ...
    def grab_batch(self, batch_idx, batch_size, inputs, targets, volatile=False):
        ...
        target_batch = Variable(
            targets[batch_idx * batch_size:(batch_idx+1) * batch_size], volatile=volatile, requires_grad=False)
...
class SingleInput_MultiTarget_Helper(object):
    ...
    def grab_batch(self, batch_idx, batch_size, inputs, targets, volatile=False):
        ...
        target_batch = [
            Variable(
                target_[batch_idx * batch_size:(batch_idx+1) * batch_size], volatile=volatile, requires_grad=False)
    ...
    def grab_batch_from_loader(self, loader_iter, volatile=False):
        ...
        return Variable(
            input_batch,
            volatile=volatile), [Variable(target_, volatile=volatile, requires_grad=False) for target_ in target_batch]
...
class MultiInput_SingleTarget_Helper(object):
    ...
    def grab_batch(self, batch_idx, batch_size, inputs, targets, volatile=False):
        input_batch = [
            Variable(input_[batch_idx * batch_size:(batch_idx+1) * batch_size], volatile=volatile) for input_ in inputs
        ]
        target_batch = Variable(
            targets[batch_idx * batch_size:(batch_idx+1) * batch_size], volatile=volatile, requires_grad=False)
        return input_batch, target_batch
    ...
    def grab_batch_from_loader(self, loader_iter, volatile=False):
        input_batch, target_batch = next(loader_iter)
        return [Variable(input_, volatile=volatile) for input_ in input_batch], Variable(
            target_batch, volatile=volatile, requires_grad=False)
