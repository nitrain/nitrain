...
def F_affine2d(x, matrix, center=True):
    ...
    coords = Variable(_coords.unsqueeze(0).repeat(x.size(0), 1, 1).float(), requires_grad=False)
    if center:
...
def F_bilinear_interp2d(input, coords):
    ...
    x_mapped = (vals_00.mul(xm).mul(ym) + vals_10.mul(xd).mul(ym) + vals_01.mul(xm).mul(yd) + vals_11.mul(xd).mul(yd))
...
def F_affine3d(x, matrix, center=True):
    ...
    # make a meshgrid of normal coordinates
    coords = Variable(th_iterproduct(x.size(1), x.size(2), x.size(3)).float(), requires_grad=False)
...
def F_trilinear_interp3d(input, coords):
    ...
    x_mapped = (
        vals_000.mul(xm).mul(ym).mul(zm) + vals_100.mul(xd).mul(ym).mul(zm) + vals_010.mul(xm).mul(yd).mul(zm) +
        vals_001.mul(xm).mul(ym).mul(zd) + vals_101.mul(xd).mul(ym).mul(zd) + vals_011.mul(xm).mul(yd).mul(zd) +
        vals_110.mul(xd).mul(yd).mul(zm) + vals_111.mul(xd).mul(yd).mul(zd))
